...
`closed="{today}"`, `closed-by` naming the criteria that resolved it, and a line in the body; a
partly resolved one is closed and the rest raised as a new gap with a new id. Only the person
closes a gap, and a gap is never deleted or reopened. `commands/crd.md` Phase 5 has the table.
