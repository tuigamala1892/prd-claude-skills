...
| That channel reaching a task | `skills/breakdown/references/task-format-spec.md :: <data-model>` | — | prd-only | **open** | **The residual V7 found, and it is narrower than the row it replaces.** The task format's `<data-model>` is defined as *the feature's own `<notes><data-model>`, carried* — PRD-only by construction. `breakdown-analyze-prd` does not mention a CRD at all, and `generate-tasks` carries `architecture.md` registry entries rather than a CRD's `<affected-contracts>`. So a schema change a CRD declares has no route into the task that implements it |
| Architectural significance flag | `schema/prd-format.md :: architecturally-significant` | — | prd-only | **open** | **Sharpened 2026-09-08 (V7): this is a reader with no producer**, which is P46's shape — the defect item 68 fixed for `<review>`. `check-references.py` reads `<architecturally-significant>`, and since group 8b `check-gate.py` runs it **for a CRD**. So the consumer is live on this path and scans for an element the CRD schema does not define. A refactor CRD is exactly what item 35 exists for. Closing it is a CRD schema change and is costed at item 75 |
