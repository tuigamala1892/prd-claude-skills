...
| A closed gap is checked against what closed it | `commands/prd.md :: --closed-since` | `commands/crd.md :: review-closures` | both | — | Different trigger, one capability. A PRD feature has a `<review at=>`, so the challenger gets the gaps closed since then; a CRD has no `<review>`, so it gets every closed gap in the document |
