...
|---|---|---|---|---|---|
| An existing PRD is found before a new one is written | `list-prds.py` | `commands/prd.md` | prd-only | Its findings need two files: `DISAGREE` compares `index.md` with `what-next.md`, `WROTE IT` reads the latter's stamp, and `touched` reads its `<last-updated>`. A CRD is one file with none of the three, and the overwrite it screens for is guarded on both paths by `check-writable.py` | 9 |
...
| `what-next.md` is derived from the PRD, never hand-maintained, stamped once, and dated on every rewrite | `build-what-next.py` | `schema/prd-format.md` · `commands/prd.md` | prd-only | `parity.md` settles deferral beyond a single document as prd-only: a CRD is one document about one change, and its deferral belongs in its own `<gaps>`. The stamp and the date are opposite rules on purpose — provenance is written once, currency is rewritten — and `list-prds.py` reads both | 11, 12, 24 |
| An artefact records the day it was written, under whichever name it uses, and a date already current is not a write | `touch-artefact.py` | `commands/prd.md` · `schema/scripts/build-what-next.py` | prd-only | Owns the date for all three artefacts that carry one — `index.md`'s `<updated>`, `what-next.md`'s `<last-updated>`, `PROJECT.md`'s — so there is one implementation rather than one per artefact. `build-what-next.py` calls it rather than keeping a copy, as it already does for `select-features.py`'s gap parser. `prd-only` is about the *callers*: a CRD is one document whose date nothing reports on | — |
