...
| Declared `<definition>` ≤ the ceiling its content supports, and a `ready` CRD carries no open `specification` gap; `<gaps>` well-formed, and aged | `check-status.py` | `commands/prd.md` · `commands/crd.md` | both | — | 3 |
