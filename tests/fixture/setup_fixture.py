...
def build(workdir):
    ...
    print("  LOADS the plugin; the second makes the plugin's own files READABLE. Without it")
    print("  the skills cannot read their own references/*.md or each other's SKILL.md, and")
    print("  the run stops partway through. NOT the scripts -- Bash is gated by the")
    print("  permission mode, not the directory allowlist, so those run either way (DP2).\n")
